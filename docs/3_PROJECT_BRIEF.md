...
| ✅ | Wio bill-pay stacking tip: shown in Path A (/optimize card matrix), Path B results (/recommend/results — within each recommended portfolio's per-category breakdown), and the /search → /recommend/[slug] category page (Job 3 — in-the-moment reference). When paying with a non-Wio card then paying that card's bill via Wio yields a higher combined return, the tip is surfaced inline. Bug fix (July 2026): tip now also fires when Wio itself is the highest direct-rate card for the category (e.g. utilities at 0.5%) — previously the check short-circuited in that case; the best non-Wio stacking card is now promoted to primary in Path A and the tip banner is shown in the search tab. Note: the 0.5% bill-pay bonus is distinct from Wio's own earn rates (2% on most categories; 0.5% on fuel/utilities/government/rent/telecom). |
...
| ✅ | Path B currency valuation disclosure (July 2, 2026): for any recommended card where the reward currency is not direct AED cashback, an inline note now shows "ⓘ 1 {currency} = AED {value} ({basis})" beneath the card's metadata in the expanded strategy view. `CURRENCY_BASIS` lookup map in `RecommendResultsClient.tsx` categorises every reward currency into one of three sourcing tiers (industry estimate / cash redemption rate / fixed program rate) so users can see the confidence level behind each valuation. Condition: `reward_currency_value_aed != null && reward_currency_value_aed !== 1`. |
| ✅ | Notes cleanup Pass 4 (July 2, 2026): Emirates Islamic Amazon World Card (18 rows) and FAB Etihad Guest Platinum/Signature/Infinite (49 rows) stripped of internal research cruft. FAB Etihad monthly earning caps moved from `card_rewards.notes` on `general` rows to `cards.summary` — card-level caps now read as card-level facts rather than category-specific restrictions. |
| ✅ | Notes cleanup Pass 5 (July 2, 2026): ENBD SHARE Visa Platinum/Signature/Infinite/Private (72 updates) — 31 rows set to null (general, airlines, insurance, rent, fuel, government, education, travel non-Private), 37 rows set to category-specific notes (dining QSR + ecosystem rate, shopping/entertainment/groceries ecosystem rates, international EU/UK carve-out, hotels room-vs-dining, utilities banking-channel exclusion, online_shopping no-ecosystem note, healthcare assumed-rate). Monthly caps moved from per-row notes to `cards.summary` for all 4 tiers. |
| ✅ | FAB du Credit Card data model fix (July 2, 2026): `utilities` `effective_return_pct` corrected from 15% → 0.15% (low-interchange MCC 4900). The du 15% co-brand rate is now stored as a 🎁 brand bonus in `card_rewards.notes` per CLAUDE.md convention — not in `effective_return_pct`. Prevents the card from misleadingly outranking general utility cards (e.g. DIB Consumer CB Platinum at 4%) in the recommendation engine for users who don't pay du bills. |
| ✅ | Path B Skywards loyalty status bug fix (July 2, 2026): Both Liv cards had `reward_currency_name = "Cashback AED / Skywards Miles"` — corrected to `"Cashback AED"`. This caused two cascading bugs: (1) the greedy algorithm applied a 1.35× Skywards loyalty weight to these cashback-only cards, making them outrank genuine Skywards cards (ENBD Skywards Infinite/Signature) whenever Skywards Gold/Silver status was selected; (2) the "Status Match" tag incorrectly appeared on Liv cards in portfolio results. `"Cashback AED / Skywards Miles"` also removed from `LOYALTY_CURRENCY_MATCH` in `lib/wallet.ts`. |
